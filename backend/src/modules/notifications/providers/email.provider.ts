...
export class EmailProvider implements IEmailProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.fromEmail =
      this.configService.get<string>('SMTP_FROM') || 'noreply@bookflow.io';
  ...
  async sendEmail(payload: EmailPayload): Promise<NotificationResult> {
    ...
    // Dev fallback: structured console log
    this.logger.log(
  ...
  /**
   * Helper: Generate RFC 5545 iCalendar (.ics) string
   */
  ...
  generateGoogleCalendarUrl(data: BookingEmailData): string {
    ...
    const title = encodeURIComponent(
      `${data.serviceName} with ${data.staffName} @ ${data.businessName}`,
    );
  ...
  /**
   * HTML Template: Customer Booking Confirmation
   */
...
<body>
  ...
  /**
   * HTML Template: Staff Assignment Alert
   */
...
<body>
  ...
  /**
   * HTML Template: Proactive Appointment Reminder (24h / 2h)
   */
...
<body>
  ...
  /**
   * HTML Template: Appointment Cancellation
   */
